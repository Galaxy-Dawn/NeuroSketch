...
class CB(nn.Module):
    def __init__(self, out_channels):
        ...
        norm_layer = partial(nn.BatchNorm2d, eps=NORM_EPS)
        self.conv3x3 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1,
...
class NCB(nn.Module):
    def __init__(self, in_channels, out_channels, stride, path_dropout=0,
                 drop=0):
    ...
    def __init__(self, cfg, stem_chs, depths, path_dropout, drop=0, use_checkpoint=False):
        ...
        for stage_id in range(len(depths)):
            ...
            for block_id in range(numrepeat):
                if cfg.model.downsample_position==1:
                    if stage_id!=0 and block_id == 0:
                        stride = 2
                    else:
                        stride = 1
                elif cfg.model.downsample_position==2:
                    if stage_id!=3 and block_id == 0:
                        stride = 2
                    else:
                        stride = 1
                elif cfg.model.downsample_position==3:
                    if stage_id==0 and block_id <= 2:
                        stride = 2
                    else:
                        stride = 1
                elif cfg.model.downsample_position==4:
...
@register_model('CNN_2D_downsample')
class CNN_2D_downsample(nn.Module):
    def __init__(self, cfg: DictConfig):
        super(CNN_2D_downsample, self).__init__()
        self.cfg = cfg
