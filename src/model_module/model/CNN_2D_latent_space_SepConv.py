...
class CB(nn.Module):
    def __init__(self, out_channels):
        ...
        norm_layer = partial(nn.BatchNorm2d, eps=NORM_EPS)
        self.pw_conv = PointwiseConv2d(out_channels, out_channels, stride=1, padding=0, bias=False)
        self.dw_conv = DepthwiseConv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
...
class Mlp(nn.Module):
    def __init__(self, in_features, out_features=None, mlp_ratio=None, drop=0., bias=True):
        ...
        hidden_dim = in_features * mlp_ratio
...
@register_model('CNN_2D_latent_space_SepConv')
class CNN_2D_latent_space_SepConv(nn.Module):
    def __init__(self, cfg: DictConfig):
        super(CNN_2D_latent_space_SepConv, self).__init__()
        self.cfg = cfg
