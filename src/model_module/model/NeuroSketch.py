...
class PatchEmbed(nn.Module):
    def __init__(self,
        ...
        if stride == 2:
            ...
            self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=2, padding=(kernel_size-1)//2, bias=False)
...
class CNN_backbone(nn.Module):
    def __init__(self, cfg, stem_chs, depths, path_dropout, use_checkpoint=False):
        ...
        self.stem = nn.Sequential(
            ConvBNReLU(1, stem_chs[0], kernel_size=cfg.model.seg_dim, stride=2),
            ConvBNReLU(stem_chs[0], stem_chs[1], kernel_size=cfg.model.seg_dim, stride=1),
            ConvBNReLU(stem_chs[1], stem_chs[2], kernel_size=cfg.model.seg_dim, stride=1),
            ConvBNReLU(stem_chs[2], cfg.model.stage1_dim, kernel_size=cfg.model.seg_dim, stride=2),
        )
